	...
	<div class="container">
		<div class="media">
			...
			<div class="media-body">
				<h3>Podgląd Rezerwacji Gokartów</h3>
			</div>
		</div>
	</div>
</div>
		...
		while($tab = $zap->fetch()){
			...
			<div class="card">
				...
				<div class="card-footer">
					<a href="?page=gokarty&s2='.$tab['id'].'" class="btn btn-lg btn-success">Sprawdź Stan Rezerwacji</a>
				</div>
			</div>';
		}
		echo "</div>";
				...
				<tbody>';
					...
					for ($i=12; $i <= 23.5; $i += 0.5) { 
						...
						if (isTrueFloat($i)) {
							...
							echo "<td class='table-success'><strong>".sprawdzNowe(zmianaDaty($aktualna->format("Y-m-d"), (int)$i, 30), $_GET['s2'])."</strong>/".$iloscGokartow."</td>";
						}
						else {
							echo "<th scope='row'>".zmianaDaty($aktualna->format("Y-m-d"), (int)$i, 0, "H")."</th>";
							echo "<td class='table-success'><strong>".sprawdzNowe(zmianaDaty($aktualna->format("Y-m-d"), (int)$i, 0), $_GET['s2'])."</strong>/".$iloscGokartow."</td>";
						}
						echo "</tr>";
					}
