...
<div class="container">
	...
	$zap = $db->query("SELECT rezerwacje.id, rezerwacje.gokart, rezerwacje.data, uzytkownicy.imie FROM rezerwacje INNER JOIN uzytkownicy ON uzytkownicy.id = rezerwacje.id_uzytkownika WHERE potwierdzenie = 0");
	while($tab = $zap->fetch()){
		echo "".$tab['imie']." | Tor: ".$tab['gokart']." | Rezerwacja na: ".$tab['data']." 
